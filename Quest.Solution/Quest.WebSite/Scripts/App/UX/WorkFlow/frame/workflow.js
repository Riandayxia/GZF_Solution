...
function getTables(connid) {
    ...
    $.ajax({
        url: ''
        //    top.rootdir + "/Platform/WorkFlowDesigner/GetTables.ashx?connid=" + connid, dataType: "json", async: false, cache: false, success: function (json)
        //{
...
function getFields(connid, table) {
    ...
    $.ajax({
        url:''
            //top.rootdir + "/Platform/WorkFlowDesigner/GetFields.ashx?connid=" + connid + "&table=" + table, dataType: "json", async: false, cache: false, success: function (json) {
            //    for (var i = 0; i < json.length; i++) {
            //        fields.push(json[i]);
            //    }
            //}
...
function subflowSetting() {
    var bbox = this.getBBox();
    var url = ''; //top.rootdir + "/Platform/WorkFlowDesigner/Set_SubFlow.aspx?appid=" + appid + "&id=" + this.id + "&x=" + bbox.x + "&y=" + bbox.y + "&width=" + bbox.width + "&height=" + bbox.height;
    dialog.open({ title: "子流程步骤参数设置", width: 700, height: 400, url: url, openerid: iframeid, resize: false });
}
...
function flowAttrSetting(isAdd) {
    var url = ''; //top.rootdir + "/Platform/WorkFlowDesigner/Set_Flow.aspx?appid=" + appid + "&isadd=" + (isAdd || 0).toString() + "&flowid=" + wf_json.id;
    dialog.open({ title: "流程属性设置", width: 550, height: 390, url: url, openerid: iframeid, resize: false });
}

//打开流程
function openFlow() {
    var url = ''; // top.rootdir + "/Platform/WorkFlowDesigner/Open.aspx?appid=" + appid;
    dialog.open({ title: "打开流程", width: 850, height: 450, url: url, openerid: iframeid, resize: false });
}

function openFlow1(id) {
    var json = $.ajax({
        url: '' //top.rootdir + "/Platform/WorkFlowDesigner/GetJSON.ashx?appid=" + appid + "&flowid=" + id, async: false, cache: false, dataType: "json", success: function (json)
        //{
        //    reloadFlow(json);
        //}
    });
...
function saveFlow(op) {
    ...
    var url = ''; // top.rootdir + "/Platform/WorkFlowDesigner/Opation.aspx?appid=" + appid + "&flowid=" + wf_json.id + "&op=" + (op || "save");
...
function saveAs() {
    ...
    var url = ''; // top.rootdir + "/Platform/WorkFlowDesigner/SaveAs.aspx?appid=" + appid + "&flowid=" + wf_json.id;
