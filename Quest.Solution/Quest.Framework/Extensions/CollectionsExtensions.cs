...
namespace SuHui.Framework.Extensions
{
    /// <summary>
    ///     集合扩展方法类
    /// </summary>
    public static class CollectionsExtensions
    {
        ...
        public static IQueryable<TEntity> Where<TEntity, TKey>(this IQueryable<TEntity> source, Expression<Func<TEntity, bool>> predicate, int pageIndex, int pageSize,
            out int total, IList<PropertySortCondition> sortConditions = null) where TEntity : BaseEntity
        {
            ...
            {
                source = source.OrderBy(m => m.CreatedTime);
            }
            else
            {
                int count = 0;
